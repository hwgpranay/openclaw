...
export function resolveSessionSharingRole(params: {
  ...
  if (params.target.entry.createdBy?.id === identity.id) {
    return "owner";
  }
...
function loadSharingSnapshot(
  ...
  const snapshot = {
    ...
    ...(target ? { creatorId: target.entry.createdBy?.id } : {}),
...
export function filterDraftSessionsForClient(params: {
  ...
  return Object.fromEntries(
    Object.entries(params.store).filter(([, entry]) => {
      return resolveSessionVisibility(entry) !== "draft" || entry.createdBy?.id === identity.id;
    }),
  );
}
